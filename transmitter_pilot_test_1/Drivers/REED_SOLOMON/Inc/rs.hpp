...
public:
     ...
     * @return RESULT_SUCCESS if successful, error code otherwise */
     int DecodeBlock(const void* src, const void* ecc, void* dst, uint8_t* erase_pos = NULL, size_t erase_count = 0, uint8_t &num_errs = NULL) {
        assert(msg_length + ecc_length < 256);
        ...
        // Find errors
        ok = FindErrors(reloc, src_len, num_errs);
        if(!ok) return 2;
     ...
     * @param erase_count  - count of known errors
     * @return RESULT_SUCCESS if successful, error code otherwise */
     int Decode(const void* src, void* dst, uint8_t* erase_pos = NULL, size_t erase_count = 0, uint8_t &num_errs = NULL) {
         const uint8_t *src_ptr = (const uint8_t*) src;
         const uint8_t *ecc_ptr = src_ptr + msg_length;

         return DecodeBlock(src, ecc_ptr, dst, erase_pos, erase_count, num_errs);
     }
...
private:
    ...
    bool FindErrors(const Poly *error_loc, size_t msg_in_size, uint8_t &num_errs) {
        Poly *err = &polynoms[ID_ERRORS];
